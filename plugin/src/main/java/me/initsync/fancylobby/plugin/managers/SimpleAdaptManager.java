...
public class SimpleAdaptManager implements AdaptManager {
	...
	public AdaptManager find() {
		...
		try {
			...
			LogUtils.error("Has happens an error to install the adapt for the server version.");
			LogUtils.error("The plugin requires the latest version of the release, example: 1.8 -> 1.8.8 (1_8_R3)");
			LogUtils.error("Or you will can use a version lower than 1.8. The plugin doesn't support versions 1.7.10 or older!");
