...
public enum FancySound {
	 ...
	 * @since 2.0.0
	 */
	public static final FancySound[] VALUES = values();
	
	public static final float DEFAULT_VOLUME = 1.0f, DEFAULT_PITCH = 1.0f;
	
	private final Sound sound;
	
	FancySound(String... legacies) {
		Sound bukkitSound = FancySound.Data.BUKKIT_NAMES.get(this.name());
		if (bukkitSound == null) {
			for (String legacy : legacies) {
				bukkitSound = FancySound.Data.BUKKIT_NAMES.get(legacy);
				if (bukkitSound != null) break;
			}
		}
		this.sound = bukkitSound;
		
		FancySound.Data.NAMES.put(this.name(), this);
		for (String legacy : legacies) {
			FancySound.Data.NAMES.putIfAbsent(legacy, this);
		}
	}
	...
	/**
	 * Parses the FancySound with the given name.
	 *
	 ...
	 * @since 1.0.0
	 */
	public static Optional<FancySound> matchSound(String sound) {
		if (sound == null || sound.isEmpty()) throw new IllegalArgumentException("Cannot match XSound of a null or empty sound name");
		return Optional.ofNullable(FancySound.Data.NAMES.get(format(sound)));
	}
	
	/**
	 * Parses the FancySound with the given bukkit sound.
	 *
	 ...
	 * @since 2.0.0
	 */
	public static FancySound matchSound(Sound sound) {
		Objects.requireNonNull(sound, "Cannot match XSound of a null sound");
		return Objects.requireNonNull(FancySound.Data.NAMES.get(sound.name()), () -> "Unsupported sound: " + sound.name());
	}
	 ...
	 * @since 1.0.0
	 */
	public static CompletableFuture<FancySound.Record> play(Player player, String sound) {
		Objects.requireNonNull(player, "Cannot play sound to null player");
		return CompletableFuture.supplyAsync(() -> {
			FancySound.Record record = parse(sound);
	 ...
	 * @since 3.0.0
	 */
	public static FancySound.Record play(Location location, String sound) {
	 ...
	 * <blockquote><pre>
	 *     Record record = FancySound.parse(player, location, sound, false).join();
	 *     // Loop:
	 ...
	 * @since 7.0.0
	 */
	public static FancySound.Record parse(String sound) {
		...
		Optional<FancySound> soundType = matchSound(name);
		...
		return new FancySound.Record(soundType.get(), null, null, volume, pitch, playAtLocation);
	}
	...
	private static final class Data {
		 ...
		 * @since 3.1.0
		 */
		private static final Map<String, FancySound> NAMES = new HashMap<>();
	...
	public static class Record {
		public final FancySound sound;
		public final float volume, pitch;
		public boolean playAtLocation;
		public Player player;
		public Location location;
		
		public Record(FancySound sound) {
			this(sound, DEFAULT_VOLUME, DEFAULT_PITCH);
		}
		
		public Record(FancySound sound, float volume, float pitch) {
			this(sound, null, null, volume, pitch, false);
		}
		
		public Record(FancySound sound, Player player, Location location, float volume, float pitch, boolean playAtLocation) {
		 ...
		 */
		public FancySound.Record forPlayer(Player player) {
			this.player = player;
			return this;
		}
		 ...
		 */
		public FancySound.Record atLocation(Location location) {
			this.location = location;
			return this;
		}
		 ...
		 */
		public FancySound.Record forPlayerAtLocation(Player player, Location location) {
			this.player = player;
			this.location = location;
			return this;
		}
