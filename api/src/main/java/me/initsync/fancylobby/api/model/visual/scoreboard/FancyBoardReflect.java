...
public class FancyBoardReflect {
	...
	public static String nmsClassName(String post1_17package, String className) {
		...
		if (!NMS_REPACKAGED) {
			return builder.append(NMS_PACKAGE).append('.').append(VERSION).append('.').append(className).toString();
		}
		
		if (post1_17package == null) return NM_PACKAGE;
		
		return builder.append(NM_PACKAGE).append('.').append(post1_17package).append('.').append(className).toString();
	}
	...
	public static FancyBoardReflect.PacketConstructor findPacketConstructor(Class<?> packetClass, MethodHandles.Lookup lookup) throws Exception {
		...
		if (theUnsafe == null) {
			synchronized (FancyBoardReflect.class) {
				if (theUnsafe == null) {
